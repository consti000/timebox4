...
- **할 일 목록** — 추가 / 완료 / 삭제, 시작~종료 기간을 지정하면 그 날짜들에 반복 표시
...
- **Google Calendar**
  ...
  - **보내기**: 연속·동일 제목 슬롯을 하나의 일정으로 병합 후 upsert. 모든 내 캘린더에서 매칭하고, 새 일정은 primary에 생성 (`timeboxOrigin=timebox4`만 관리, 기존 일정 삭제 없음)
...
- **한 날짜 ≈ 한 페이지**를 목표로: 여백 축소, 글자·행간 압축, 타임박스 **좌·우 2단(4열)** 배치(빈 슬롯 유지).
...
| 동작 | 동작 방식 |
|------|-----------|
| 불러오기 | 쓰기 가능한 **내 캘린더 전체**(primary·Money·Privacy 등)에서 timed 이벤트 → 빈 타임박스 슬롯만 채움. **종일 일정 → 할 일 목록에 추가**(동일 제목 중복 제외). Timebox4가 만든 일정은 무시 |
| 보내기 | 모든 내 캘린더에서 제목+시작(또는 시간 겹침)으로 매칭. Timebox4면 해당 캘린더에서 수정, 외부면 생성 생략, 새 일정은 primary에 생성. **기존 일정은 삭제하지 않음** |
| 자동 동기화 | 없음 (버튼 수동) |
